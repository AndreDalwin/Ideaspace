...
- `packages/ideaspace`: runtime/backend used by the desktop app
...
By default, `bun dev` runs Ideaspace in the `packages/ideaspace` directory. To run it against a different directory or repository:
...
```bash
./packages/ideaspace/script/build.ts --single
```

Then run it with:

```bash
./packages/ideaspace/dist/ideaspace-<platform>/bin/ideaspace
```
...
> [!NOTE]
> If you make changes to the API or SDK (e.g. `packages/ideaspace/src/server/server.ts`), run `./packages/sdk/js/script/build.ts` to regenerate the JavaScript SDK.
...
- If `spawn` does not work for you, you can debug the server separately:
  - Debug server: `bun run --inspect=ws://localhost:6499/ --cwd packages/ideaspace ./src/index.ts serve --port 4096`,
    then attach TUI with `opencode attach http://localhost:4096`
  - Debug TUI: `bun run --inspect=ws://localhost:6499/ --cwd packages/ideaspace --conditions=browser ./src/index.ts`
...
Blank issues are not allowed. Please follow the template and include enough context for someone else on the team to understand the problem or request without extra back-and-forth.
...
If you are unsure whether an issue belongs here, ask a maintainer before opening a large feature request or broad product change.
