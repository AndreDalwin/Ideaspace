...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("ideaspace://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("ideaspace://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("ideaspace://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("ideaspace://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("ideaspace://open-project")).toBeUndefined()
    expect(parseDeepLink("ideaspace://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "ideaspace://open-project?directory=/a",
      "ideaspace://other?directory=/b",
      "ideaspace://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("ideaspace://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("ideaspace://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("ideaspace://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("ideaspace://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "ideaspace://new-session?directory=/a",
      "ideaspace://open-project?directory=/b",
      "ideaspace://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __IDEASPACE__: {
        deepLinks: ["ideaspace://open-project?directory=/a"],
      },
    } as unknown as Window & { __IDEASPACE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["ideaspace://open-project?directory=/a"])
