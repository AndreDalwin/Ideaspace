...
const TUI_SCHEMA_URL = "https://ideaspace.ai/tui.json"
...
async function configFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.IDEASPACE_DISABLE_PROJECT_CONFIG
    ? []
    : (
        await Promise.all(
          ["ideaspace"].map((name) => ConfigPaths.projectFiles(name, Instance.directory, Instance.worktree)),
        )
      ).flat()
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "ideaspace")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "ideaspace"))
  }
  if (Flag.IDEASPACE_CONFIG) files.push(Flag.IDEASPACE_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "ideaspace"))
