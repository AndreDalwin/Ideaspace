...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.IDEASPACE_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.IDEASPACE_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.IDEASPACE_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".ideaspace") && dir !== Flag.IDEASPACE_CONFIG_DIR) continue
