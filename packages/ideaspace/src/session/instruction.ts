...
function globalFiles() {
  ...
  if (Flag.IDEASPACE_CONFIG_DIR) {
    files.push(path.join(Flag.IDEASPACE_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.IDEASPACE_DISABLE_CLAUDE_CODE_PROMPT) {
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.IDEASPACE_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no IDEASPACE_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.IDEASPACE_CONFIG_DIR, Flag.IDEASPACE_CONFIG_DIR).catch(() => [])
}
