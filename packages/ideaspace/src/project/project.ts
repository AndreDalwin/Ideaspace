  ...
  export async function fromDirectory(directory: string) {
    ...
    const data = await iife(async () => {
      ...
      if (dotgit) {
        ...
        let id = await Filesystem.readText(path.join(dotgit, "ideaspace"))
          .then((x) => x.trim())
          .catch(() => undefined)
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox: sandbox,
            vcs: Info.shape.vcs.parse(Flag.IDEASPACE_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          id = roots[0]
          if (id) {
            await Filesystem.write(path.join(dotgit, "ideaspace"), id).catch(() => undefined)
          }
        }
    ...
    if (Flag.IDEASPACE_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
