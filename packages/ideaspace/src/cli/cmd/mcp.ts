...
async function resolveConfigPath(baseDir: string, global = false) {
  const candidates = [path.join(baseDir, "ideaspace.jsonc"), path.join(baseDir, "ideaspace.json")]

  if (!global) {
    candidates.push(
      path.join(baseDir, ".ideaspace", "ideaspace.jsonc"),
      path.join(baseDir, ".ideaspace", "ideaspace.json"),
    )
  }
