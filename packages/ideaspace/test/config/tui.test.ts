...
afterEach(async () => {
  delete process.env.IDEASPACE_CONFIG
  delete process.env.IDEASPACE_TUI_CONFIG
...
test("project config takes precedence over IDEASPACE_TUI_CONFIG (matches IDEASPACE_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.IDEASPACE_TUI_CONFIG = custom
    },
  })
...
test("IDEASPACE_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from IDEASPACE_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.IDEASPACE_CONFIG = path.join(customDir, "opencode.json")
    },
  })
