...
const managedConfigDir = process.env.IDEASPACE_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => undefined)
})

async function writeManagedSettings(settings: object, filename = "ideaspace.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "ideaspace.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "ideaspace.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "ideaspace.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .ideaspace directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const ideaspaceDir = path.join(dir, ".ideaspace")
      await fs.mkdir(ideaspaceDir, { recursive: true })
      const agentDir = path.join(ideaspaceDir, "agent")
...
test("loads agents from .ideaspace/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const ideaspaceDir = path.join(dir, ".ideaspace")
      await fs.mkdir(ideaspaceDir, { recursive: true })

      const agentsDir = path.join(ideaspaceDir, "agents")
...
test("loads commands from .ideaspace/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const ideaspaceDir = path.join(dir, ".ideaspace")
      await fs.mkdir(ideaspaceDir, { recursive: true })

      const commandDir = path.join(ideaspaceDir, "command")
...
test("loads commands from .ideaspace/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const ideaspaceDir = path.join(dir, ".ideaspace")
      await fs.mkdir(ideaspaceDir, { recursive: true })

      const commandsDir = path.join(ideaspaceDir, "commands")
...
test("updates config and writes to file", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const next = { model: "updated/model" } satisfies Config.Info
...
test("does not try to install dependencies in read-only IDEASPACE_CONFIG_DIR", async () => {
  ...
  await using tmp = await tmpdir<string>({
    ...
    dispose: async (dir) => {
      const ro = path.join(dir, "readonly")
      await fs.chmod(ro, 0o755).catch(() => undefined)
      return ro
    },
  })

  const prev = process.env.IDEASPACE_CONFIG_DIR
  process.env.IDEASPACE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.IDEASPACE_CONFIG_DIR
    else process.env.IDEASPACE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable IDEASPACE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "ideaspace.json"),
        JSON.stringify({ $schema: "https://ideaspace.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "ideaspace.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .ideaspace config
      const projectDir = path.join(dir, "project")
      const ideaspaceDir = path.join(projectDir, ".ideaspace")
      await fs.mkdir(ideaspaceDir, { recursive: true })
      ...
      // Local .ideaspace config with different plugins
      await Filesystem.write(
        path.join(ideaspaceDir, "ideaspace.json"),
        JSON.stringify({
          $schema: "https://ideaspace.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .ideaspace config with some overlapping plugins
      await Filesystem.write(
        path.join(ideaspaceDir, "ideaspace.json"),
        JSON.stringify({
          $schema: "https://ideaspace.ai/config.json",
...
// Note: preload.ts sets IDEASPACE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .ideaspace config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .ideaspace directory config enables it
      const ideaspaceDir = path.join(dir, ".ideaspace")
      await fs.mkdir(ideaspaceDir, { recursive: true })
      await Filesystem.write(
        path.join(ideaspaceDir, "ideaspace.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/ideaspace")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/ideaspace")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-ideaspace@2.4.3")).toBe("oh-my-ideaspace")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@ideaspace/plugin@2.0.0")).toBe("@ideaspace/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-ideaspace@2.4.3", "file:///project/.ideaspace/plugin/oh-my-ideaspace.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.ideaspace/plugin/oh-my-ideaspace.js")
  })
  ...
  test("local plugin directory overrides global ideaspace.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const ideaspaceDir = path.join(projectDir, ".ideaspace")
        const pluginDir = path.join(ideaspaceDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("IDEASPACE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["IDEASPACE_DISABLE_PROJECT_CONFIG"]
    process.env["IDEASPACE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["IDEASPACE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["IDEASPACE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .ideaspace/ directories when flag is set", async () => {
    const originalEnv = process.env["IDEASPACE_DISABLE_PROJECT_CONFIG"]
    process.env["IDEASPACE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .ideaspace directory with a command
          const ideaspaceDir = path.join(dir, ".ideaspace", "command")
          await fs.mkdir(ideaspaceDir, { recursive: true })
          await Filesystem.write(path.join(ideaspaceDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .ideaspace should NOT be in directories list
          const hasProjectIdeaspace = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectIdeaspace).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["IDEASPACE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["IDEASPACE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["IDEASPACE_CONFIG_DIR"]
      process.env["IDEASPACE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["IDEASPACE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["IDEASPACE_CONFIG_DIR"]
      } else {
        process.env["IDEASPACE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("IDEASPACE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["IDEASPACE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["IDEASPACE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["IDEASPACE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["IDEASPACE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from IDEASPACE_CONFIG_DIR, not project
...
describe("IDEASPACE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in IDEASPACE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["IDEASPACE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["IDEASPACE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://ideaspace.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["IDEASPACE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["IDEASPACE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in IDEASPACE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["IDEASPACE_CONFIG_CONTENT"]
