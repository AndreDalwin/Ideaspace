...
Replace runtime Bun shell template-tag usage in `packages/ideaspace/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/ideaspace/src` is removed except:
