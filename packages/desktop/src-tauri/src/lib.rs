...
async fn initialize(app: AppHandle) {
    ...
    let sqlite_done = needs_sqlite_migration.then(|| {
        tracing::info!(
            path = %ideaspace_data_dir().expect("failed to get Ideaspace data directory").display(),
            "Sqlite file not found in Ideaspace data directory, waiting for it to be generated"
        );
...
fn sqlite_file_exists() -> bool {
    let data_dir = match ideaspace_data_dir() {
        Ok(dir) => dir,
        Err(_) => return true,
    };
...
fn ideaspace_data_dir() -> Result<PathBuf, &'static str> {
