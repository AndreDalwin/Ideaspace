...
const deepLinkEvent = "ideaspace:deep-link"

const state = () => {
  const boot = window.__IDEASPACE__ ?? {}
  window.__IDEASPACE__ = boot
  return boot
}
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !state().wsl) return undefined
    return commands.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !state().wsl) return result
